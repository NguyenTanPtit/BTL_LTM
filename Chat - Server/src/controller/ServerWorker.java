...
public class ServerWorker {
    ...
    public void checkLogin(String username, String password, String actionType) {
        ...
        if (user != null) {
            ...
            Response response = new Response(actionType, ResultFlags.ERROR, "Wrong username or password", null);
            send(response);
        }
    }
    ...
    private boolean checkExistsUserRoom(User user, Room room) {
        return room.getListUserRoom().stream().anyMatch(userRoom -> (user.getId() == userRoom.getUser().getId()));
    }
    ...
    public Room addUserToRoom(UserRoom userRoom) {
        ...
        if (checkExistsUserRoom(userRoom.getUser(), room)) {
            Response response = new Response(ActionFlags.ADD_USER_TO_ROOM, ResultFlags.ERROR, "User already exist in room", null);
            send(response);
            return null;
        }
    ...
    public void checkRegister(User user, String actionType) {
        if (!userDAO.checkExistsUser(user.getUsername())) {
            ...
            Response response = new Response(actionType, ResultFlags.ERROR, "Username already exist", null);
            send(response);
        }
    }
...
//            room.setDescription("Room chat cua: " + user.getDisplayName() + " va " + userc.getDisplayName());
            room.setDescription( user.getDisplayName() );
